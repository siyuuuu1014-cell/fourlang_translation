...
def main():
    ...
    # Leakage policy
    #
    # Validation / split / 17C teacher-selection overlap means the
    # frozen split itself is broken -> hard abort.
    #
    # Benchmark overlap is handled differently:
    # explicitly isolate the affected TRAIN pairs from the KD pool,
    # preserve the original Step15A split unchanged, then re-audit.
    # --------------------------------------------------------

    hard_invariant_keys = [
        "validation_pair_id_overlap",
        "validation_group_overlap",
        "validation_exact_pair_overlap",
        "validation_english_overlap",
        "validation_chinese_overlap",
        "teacher_selection_pair_id_overlap",
        "teacher_selection_exact_pair_overlap",
        "teacher_selection_english_overlap",
        "teacher_selection_chinese_overlap",
    ]

    hard_invariant_failures = {
        key: int(leakage[key])
        for key in hard_invariant_keys
        if leakage[key] != 0
    }

    if hard_invariant_failures:
        raise RuntimeError(
            "\nSTEP 18A ABORTED: frozen split / teacher-selection leakage detected.\n"
            + json.dumps(
                hard_invariant_failures,
                ensure_ascii=False,
                indent=2,
    ...
    # Identify every train pair that touches a frozen benchmark on
    # exact pair OR either language sentence.
    benchmark_exclusion_mask = []

    for _, row in train_pairs.iterrows():
        en_n = normalize_text(row["en"])
        zh_n = normalize_text(row["zh"])
        p_key = make_pair_key(row["en"], row["zh"])

        overlap_exact = p_key in benchmark_pair_keys
        overlap_en = en_n in benchmark_en
        overlap_zh = zh_n in benchmark_zh

        benchmark_exclusion_mask.append(
            bool(
                overlap_exact
                or overlap_en
                or overlap_zh
    ...
    benchmark_exclusion_mask = pd.Series(
        benchmark_exclusion_mask,
        index=train_pairs.index,
        dtype=bool,
    )
    ...
    excluded_pair_ids = set(
        excluded_pairs["pair_id"]
        .astype(str)
        .tolist()
    )

    clean_train_directed = (
        train_directed.loc[
            ~train_directed["pair_id"]
            .astype(str)
            .isin(excluded_pair_ids)
        ]
        .copy()
        .reset_index(drop=True)
    )

    # Add explicit reasons to the isolated records.
    if len(excluded_pairs):
        excluded_pairs["benchmark_exact_pair_overlap"] = [
            make_pair_key(en, zh) in benchmark_pair_keys
            for en, zh in zip(
                excluded_pairs["en"],
                excluded_pairs["zh"],
            )
        ...
        excluded_pairs["benchmark_english_overlap"] = [
            normalize_text(en) in benchmark_en
            for en in excluded_pairs["en"]
        ]

        excluded_pairs["benchmark_chinese_overlap"] = [
            normalize_text(zh) in benchmark_zh
            for zh in excluded_pairs["zh"]
        ]

        excluded_pairs["exclusion_reason"] = [
            "|".join(
                reason
                for reason, flag in [
                    ("BENCHMARK_EXACT_PAIR", exact_flag),
                    ("BENCHMARK_EN_SENTENCE", en_flag),
                    ("BENCHMARK_ZH_SENTENCE", zh_flag),
                ]
                if flag
            )
            for exact_flag, en_flag, zh_flag
            in zip(
                excluded_pairs["benchmark_exact_pair_overlap"],
                excluded_pairs["benchmark_english_overlap"],
                excluded_pairs["benchmark_chinese_overlap"],
    ...
    print(
        "\nBenchmark-isolated train pairs:",
        len(excluded_pairs),
    )
    ...
    if len(excluded_pairs):
        show_cols = [
            ...
            if c in excluded_pairs.columns
        ]
    ...
    # --------------------------------------------------------
    # Re-audit AFTER benchmark isolation.
    # Final KD pool must have ZERO overlap at pair / EN / ZH level.
    # --------------------------------------------------------

    clean_pair_ids = set(
        clean_train_pairs["pair_id"]
        .astype(str)
        .tolist()
    )

    clean_groups = set(
        clean_train_pairs["split_group_id"]
        .astype(str)
        .tolist()
    )

    clean_en = set(
        clean_train_pairs["en"]
        .map(normalize_text)
        .tolist()
    )

    clean_zh = set(
        clean_train_pairs["zh"]
        .map(normalize_text)
        .tolist()
    )
    ...
    final_leakage = {
        "validation_pair_id_overlap": len(
            clean_pair_ids & validation_pair_ids
        ),
        "validation_group_overlap": len(
            clean_groups & validation_groups
        ),
        "validation_exact_pair_overlap": len(
            clean_pair_keys & validation_pair_keys
        ),
        "validation_english_overlap": len(
            clean_en & validation_en
        ),
        "validation_chinese_overlap": len(
            clean_zh & validation_zh
        ),

        "benchmark_exact_pair_overlap": len(
            clean_pair_keys & benchmark_pair_keys
        ),
        "benchmark_english_overlap": len(
            clean_en & benchmark_en
        ),
        "benchmark_chinese_overlap": len(
            clean_zh & benchmark_zh
        ),

        "teacher_selection_pair_id_overlap": len(
            clean_pair_ids & teacher_pair_ids
        ),
        "teacher_selection_exact_pair_overlap": len(
            clean_pair_keys & teacher_pair_keys
        ),
        "teacher_selection_english_overlap": len(
            clean_en & teacher_en
        ),
    ...
    print(
        "\nFinal leakage audit after benchmark isolation:"
    )

    for key, value in final_leakage.items():
        print(
            f"{key}: {value}"
        )
    ...
    if final_nonzero:
        raise RuntimeError(
            "\nSTEP 18A V2 ABORTED: leakage remains after benchmark isolation.\n"
            + json.dumps(
                final_nonzero,
                ensure_ascii=False,
                indent=2,
    ...
    # From this point onward, ONLY the cleaned train-only pool is used.
    train_pairs = clean_train_pairs
    train_directed = clean_train_directed
    nonzero_leakage = final_nonzero
    leakage_before_filter = leakage
    ...
    assertions = {
        ...
        "final_pair_count_matches_directed": (
            len(directed_out) == 2 * len(pair_out)
        ),
        "no_empty_source": (
    ...
    excluded_output = (
        output_dir
        / "benchmark_excluded_train_pairs_v1.parquet"
    )

    excluded_csv_output = (
        output_dir
        / "benchmark_excluded_train_pairs_v1.csv"
    )
    ...
    report = {
        ...
        "counts": {
            "input_train_pairs_before_benchmark_isolation": int(
                len(pair_out) + len(excluded_pairs)
            ),
            "benchmark_excluded_pairs": int(
                len(excluded_pairs)
            ),
            "benchmark_excluded_directed_rows": int(
                len(excluded_pairs) * 2
            ),
        ...
        "leakage_before_benchmark_isolation": {
            str(k): int(v)
            for k, v
            in leakage_before_filter.items()
        },
        ...
        "benchmark_isolation": {
            ...
            "original_step15a_files_modified": False,
        },
        ...
        "outputs": {
            ...
            "benchmark_excluded_pairs": str(
                excluded_output
            ),
            "benchmark_excluded_pairs_csv": str(
                excluded_csv_output
            ),
        },
    ...
    print("Excluded directed rows:", len(excluded_pairs) * 2)

    if len(excluded_pairs):
        print("Excluded pair IDs:")
        for pair_id in excluded_pairs["pair_id"].astype(str):
            print("-", pair_id)
    ...
    print("\nBenchmark-excluded train pairs:")
