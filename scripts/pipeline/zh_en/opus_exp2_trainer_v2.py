...
STEP_VERSION = "19_EXP2_KD_V2_WEIGHT_ABLATION"
DEFAULT_SEED = 2026
...
VALID_DIRECTIONS = {
    "zh_en": {
        ...
        "output_relative": (
            "results/specialists/"
            "zh_en/opus_mt_zh_en/exp2_kd_v2"
        ),
    },
    ...
    "en_zh": {
        ...
        "output_relative": (
            "results/specialists/"
            "en_zh/opus_mt_en_zh/exp2_kd_v2"
        ),
    },
}
...
def build_parser(
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Training loss weight for HUMAN_REPLAY rows. "
            "Step19B default = 1.0."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.5,
        help=(
            "Training loss weight for TEACHER_KD rows. "
            "Step19B clean ablation default = 0.5."
        ),
    )
...
def run_direction(
    ...
    print("STEP 19B - EXP2 V2 WEIGHTED HUMAN REPLAY + MULTI-TEACHER KD")
    print("=" * 110)
    ...
    # ========================================================
    # Step19B clean weight ablation
    # ========================================================
    # IMPORTANT:
    # Do not mutate the frozen 18H parquet. Override weights only
    # inside this experiment so V1 and V2 remain directly comparable.
    if args.human_weight <= 0:
        raise ValueError("--human_weight must be > 0.")

    if args.kd_weight <= 0:
        raise ValueError("--kd_weight must be > 0.")
    ...
    if unknown_origins:
        raise RuntimeError(
            "Unexpected training_origin values: "
    ...
    for epoch in range(
        ...
        # Persist every trained epoch candidate regardless of
        # cross-entropy ranking. Step20 will arbitrate using BLEU/chrF++.
        epoch_model_dir = (
            epoch_models_root
            /
            f"epoch_{epoch}"
    ...
    report = {
        ...
        "status":
            (
                "EXP2_V2_IMPROVED_VALIDATION"
                if best_epoch > 0
                else
                "EXP1_BASELINE_REMAINS_BEST_AFTER_EXP2_V2"
            ),
    }
...
if __name__ == "__main__":
    raise RuntimeError(
        "Do not run opus_exp2_trainer_v2.py directly. "
        "Use 19a_train_opus_zh_en_exp2_v2.py or "
        "19b_train_opus_en_zh_exp2_v2.py."
    )
